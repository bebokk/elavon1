...
//load all actions done for this ticket	
$query = "SELECT * FROM tickets_actions WHERE ".$_SESSION['table_key']."='".$_POST['element_id']."'";
$result = @mysql_query ($query);	
$tickets_actions_tab='';
while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {
	foreach ($row as $key => $value) {		
		$tickets_actions_tab[$row['tickets_actionid']][$key]=$value;
	}
}
...
while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {
	...
	$users2_tab[$row['userid']]=$row['color'];
}
				...
				<div class="popup1_content1">
					<div class="popup1_content1_1">
						...
						<div class="popup1_content1_1_3">
							<table>
								<tr>
									<td colspan="2">Changes logs</td>
								</tr>	
								<? if ($tickets_actions_tab != '') foreach ($tickets_actions_tab as $key => $value) { ?>
									<tr>
										<td>
											<div class="tickets_box1" style="background-color:<? echo $users2_tab[$tickets_actions_tab[$key]['userid']]; ?> !important;">
												Action:  <b><? echo $tickets_states_tab[$tickets_actions_tab[$key]['state']]; ?></b> <br />
												By:  <b><? echo $users_tab[$tickets_actions_tab[$key]['userid']]; ?></b> <br />
												Date:  <b><? echo $tickets_actions_tab[$key]['createtime']; ?></b>
											</div>
										</td>
										<td>
											<div class="tickets_box2">
												<? echo $tickets_actions_tab[$key]['decription']; ?>
											</div>
										</td>
									</tr>	
								<? } ?>								
