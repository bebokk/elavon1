...
$query = "SELECT * FROM structure_basic_pictures WHERE table_name='".$_SESSION['table_name']."' AND tableid='".$_POST['element_id']."'";		
$result = @mysql_query ($query);		
while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {
	foreach ($row as $key => $value) {
...
//include gallery
$query = "SELECT * FROM structure_basic_pictures WHERE table_name='".$_SESSION['table_name']."' AND
...
if ($pictures_details_tab != '') foreach ($pictures_details_tab as $key => $value) {
							...
							<div class="details1">
								<div class="pictures_add">
									<div class="pictures_add1">
										Upload new pictures <input type="file" min="1" max="20" multiple name="new_pictures_tab[]" onchange="upload_picture(<? echo $_POST['element_id']; ?>);">
									</div>
								</div>
								<div class="pictures">
