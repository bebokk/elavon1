...
<div class="back">
	<a href="index.php?page={$page}&subpage={$subpage}&id1={$id1}">
		<input type="button" class="button_look1" value="{$trans_tab.1.9}" />
	</a>
</div>
<div class="structure1">
	<form action="index.php?page={$page}&subpage={$subpage}&id1={$id1}&details={$details}" method="POST">
		<div class="header1"><span>{$trans_tab.1.10} {$page_tab.name}</span></div>
		<div class="details1">
		...
		<div class="save">
			<input class="button_look1" type="submit" name="save_details" value="{$trans_tab.1.15}" />
		</div>
	</form>
