...
function element_edit(element_id) {	
	...
	loading01();
	$.post("php/popups/contact_logs/structure_basic.php", {'element_id': element_id},  function(data){
		$(".popup0").html(data);
		$(".focus_here").focus();
		tiny_mce_on();
		loading02();
...
function save_element_changes_1(element_id) {	
	
	var nextactiondate = $("#nextactiondate").val();
	var nextactiontime = $("#nextactiontime").val();
	var timetospend = $("#timetospend").val();
	
	//check data time availability 	
	$.post("php/popups/contact_logs/check_time.php", {'element_id': element_id,'nextactiondate': nextactiondate,'nextactiontime': nextactiontime,
	'timetospend': timetospend},  function(data){	
	
		//alert('data -- '+data);
		if (data == 0) {
			save_element_changes(element_id);
		} else {
