...
if ($_POST['action'] == 'clear_bigselect_element') {
	...
	foreach ($_POST['save_parameters_id_tab'] as $key => $value) {
		if ($elements_details1_tab['type'][$value] != 'pass' OR $_POST['save_parameters_val_tab'][$key] != '') {
			...
			if ($value == 'userid') $userid1=$_POST['save_parameters_val_tab'][$key];
			if ($value == 'state') $state1=$_POST['save_parameters_val_tab'][$key];
			if ($value == 'decription') $decription1=$_POST['save_parameters_val_tab'][$key];
		} 
	}
	...
	//for tickets save every action for logs	
	$query = "INSERT INTO tickets_actions (tickets_actionid,ticketid,userid,state,decription,createtime)
	VALUES
	(NULL,'".$_POST['element_id']."','".$userid1."','".$state1."','".$decription1."','".date("Y-m-d H:i:s")."')";					
	$result = mysql_query ($query); 	 
	//echo "query -- $query -- $result <br>";
