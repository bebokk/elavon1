...
if ($_POST['action'] == 'clear_bigselect_element') {
	...
	foreach ($_POST['save_parameters_id_tab'] as $key => $value) {
		if ($elements_details1_tab['type'][$value] != 'pass' OR $_POST['save_parameters_val_tab'][$key] != '') {
			if ($elements_details1_tab['type'][$value] == 'multiselect') {
				...
				$query1 .= ",`".$value."`='".$val1."'";		
				$urltitle3=get_cat_name_link($val1);	
				/*
				echo "urltitle3 -- <pre>";
				print_r($urltitle3);
				echo "</pre>";
				*/
				...
				$urltitle1=wyrzuc_polskie_i_duze_litery($_POST['save_parameters_val_tab'][$key]);	
			} elseif ($value == 'publication_date') {
				$query1 .= ",`".$value."`='".$_POST['save_parameters_val_tab'][$key]."'";	
				$urltitle2=$_POST['save_parameters_val_tab'][$key];					
			} else {
				$query1 .= ",`".$value."`='".$_POST['save_parameters_val_tab'][$key]."'";		
			}
		} 
	...
	$query1 .= ",lastactiontime='".date("Y-m-d H:i:s")."',urltitle='".wyrzuc_polskie_i_duze_litery($urltitle3['name'])."/".$urltitle2."/".$urltitle1.".html'";		
	$query .= $query1;
