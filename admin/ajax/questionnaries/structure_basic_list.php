...
<table class="list1_1" cellspacing="0">	
	<thead>
		...
		<tr class="list1_1_sort">
			...
			<? } ?>			
			<td style="width:30px;" class="tcenter"><a href="#"><span>Csv</span></a></td>
		...
		<? if ($data_tab1 != '') { ?>
			<? foreach ($data_tab1 as $id => $val) { ?>
				...
				<? } ?>
									...
									<span id="<? echo $id1; ?>_<? echo $id; ?>">
										...
										foreach ($val001 as $key0001 => $value0001) {
											if ($value0001 != '') {
												...
												?><? echo $elements_list1_tab['select'][$id1][$value0001]; ?>
												<? 
											} 
										} ?>
									</span>
					...
					<? } ?>			
					<td class="tcenter list1_1_1" style="width:30px;"><a href="csv/questionnaire.php?id=<? echo $id; ?>"><span>GET&nbsp;CSV</span></a></td>
					<? if ($_SESSION['page_settings_tab']['translations'] == 1) { ?>					
